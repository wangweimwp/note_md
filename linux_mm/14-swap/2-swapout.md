        ...
        struct reclaim_stat *stat, bool ignore_references)
{
    ...
    .......
 /*解映射关系，并修改pte，使其值等于 page->private即swp_entry_t变量，
*等到swapin 时就直接把pte强制类型转换成swp_entry_t 
*/
    try_to_unmap(folio, flags);//解除映射
    .....
    /*发生swapout的写请求，并清除page的dirty标记，
    在__swap_writepage时设置writeback标记， 当io完成后writeback才会被清除*/
...
add_to_swap
    ...
    ->add_to_swap_cache//将page放到address_space的xarry树中
    ->folio_mark_dirty(folio);//看代码注释
```
        ...
        ->__swap_writepage
            ->if (data_race(sis->flags & SWP_FS_OPS))
                ...
                swap_writepage_bdev_sync(folio, wbc, sis);
                    ->submit_bio_wait(&bio);//向block层发送bio，等待bio
              else
                swap_writepage_bdev_async(folio, wbc, sis);
                    ->folio_start_writeback(folio);
                        ->folio_test_set_writeback(folio);//设置writeback标记
                    ->submit_bio(bio);//向block层发送bio
```
